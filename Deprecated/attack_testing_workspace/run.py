...
if(args.model == True):
    if(args.run == None): 
        runs = 15
    os.system("sh scp_helper.sh {0} {1} {2} {3} {4} 1".format(wd,source_file,name,runs,wait_time))
    processMLData(runs,name)
    os.system("sh model_helper.sh {0} {1} {2}".format(wd,name+"/"+"X_attack_test_15.csv",name))
    with open("{0}{1}/results_{1}.txt".format(wd,name), 'r') as f:
        print(f.read())

# Simply run attack # amount of times with # seconds between each attack
else:
    os.system("sh scp_helper.sh {0} {1} {2} {3} {4} 0".format(wd,source_file,name,runs,wait_time))
